...
describe("basic tests", () => {
    ...
    test("0_getNetwork", async () => {
        const n = await index_1.default.getNetwork();
        expect(n).toBe('testnet');
    });
    test("1_getTransactionOutputs", async () => {
        try {
            await index_1.default.getTransactionOutputs({});
        }
        catch (e) {
        ...
        let outs = await index_1.default.getTransactionOutputs({ basket: 'foobar' });
        if (outs.length === 0) {
            const grantorKey = 'eeeeeeeeeeeeeeeeeeeeeeeeeeeeeafeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee';
            const lockScript = await pushdrop_1.default.create({
                fields: [
                    Buffer.from('1'),
                    Buffer.from('2'),
                    Buffer.from('3'),
                ],
                key: sdk_1.PrivateKey.fromString(grantorKey, 'hex').toWif()
            });
            const r = await index_1.default.createAction({
                outputs: [{ basket: 'foobar', satoshis: 101, script: lockScript }],
                description: 'test',
                acceptDelayedBroadcast: true
            });
            expect((0, cwi_base_1.verifyTruthy)(r.txid).length).toBe(64);
            outs = await index_1.default.getTransactionOutputs({ basket: 'foobar' });
        }
    ...
    test("2_encrypt and decrypt", async () => {
        ...
        {
            const ciphertext = await index_1.default.encrypt({ plaintext, protocolID: [2, 'foobar'], keyID: '42', returnType: 'string' });
            const decrypted = await index_1.default.decrypt({ ciphertext, protocolID: [2, 'foobar'], keyID: '42', returnType: 'string' });
            expect(decrypted).toBe(plaintext);
        }
        {
            const ciphertext = await index_1.default.encrypt({ plaintext, protocolID: [2, 'foobar'], keyID: '42', returnType: 'string' });
            const decrypted = await index_1.default.decrypt({ ciphertext, protocolID: [2, 'foobar'], keyID: '42', returnType: 'Uint8Array' });
            const decryptext = new TextDecoder().decode(decrypted);
            expect(decryptext).toBe(plaintext);
        }
        {
            const ciphertext = await index_1.default.encrypt({ plaintext, protocolID: [2, 'foobar'], keyID: '42', returnType: 'Uint8Array' });
            const decrypted = await index_1.default.decrypt({ ciphertext, protocolID: [2, 'foobar'], keyID: '42', returnType: 'string' });
            expect(decrypted).toBe(plaintext);
        }
        {
        ...
        {
            const ciphertext = await index_1.default.encrypt({ plaintext: plaintextA, protocolID: [2, 'foobar'], keyID: '42', returnType: 'string' });
            const decrypted = await index_1.default.decrypt({ ciphertext, protocolID: [2, 'foobar'], keyID: '42', returnType: 'string' });
            expect(decrypted).toBe(plaintext);
        }
        {
        ...
        {
            const ciphertext = await index_1.default.encrypt({ plaintext: plaintextA, protocolID: [2, 'foobar'], keyID: '42', returnType: 'Uint8Array' });
            const decrypted = await index_1.default.decrypt({ ciphertext, protocolID: [2, 'foobar'], keyID: '42', returnType: 'string' });
            expect(decrypted).toBe(plaintext);
        }
        {
    ...
    test("3_getPublicKey", async () => {
        const keyID = (0, cwi_base_1.randomBytesBase64)(32);
        const publicKey = await index_1.default.getPublicKey({ protocolID: 'demoo', keyID });
