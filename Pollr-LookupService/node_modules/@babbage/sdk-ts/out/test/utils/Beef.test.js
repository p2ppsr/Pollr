...
exports.BEEF_MAGIC_V2 = exports.BEEF_MAGIC = void 0;
...
describe('Beef tests', () => {
    ...
    const chainTracker = {
        ...
        currentHeight: async () => { return 1631619; }
    };
    test('0_mergeTransaction', async () => {
        const beef = sdk_1.Beef.fromString(beefs[0]);
        expect(beef.toHex()).toBe(beefs[0]);
        expect(beef.toLogString()).toBe(beefs0Log);
        expect(sdk_1.Beef.fromBinary(beef.toBinary()).toLogString()).toBe(beefs0Log);
        const btx = beef.txs[0];
        beef.txs[0] = new sdk_1.BeefTx(beef.txs[0].tx, 0);
        expect(sdk_1.Beef.fromBinary(beef.toBinary()).toLogString()).toBe(beefs0Log);
        ...
        beef.txs[0] = btx;
        const tx = sdk_1.Transaction.fromHex(txs[0]);
        beef.mergeTransaction(tx);
        beef.sortTxs();
        expect(beef.toLogString()).toBe(log2);
        {
            const beef = sdk_1.Beef.fromString(beefs[0]);
            beef.mergeTransaction(sdk_1.Transaction.fromHex(txs[0]));
        ...
        {
            const btx = new sdk_1.BeefTx(tx.toBinary());
            expect(btx.rawTx).toEqual(tx.toBinary());
        }
        {
            const btx = new sdk_1.BeefTx('bd4a39c6dce3bdd982be3c67eb04b83934fd431f8bcb64f9da4413c91c634d07');
        ...
        }
        {
            ...
            const beef0 = sdk_1.Beef.fromString(beefs[0]);
        ...
        {
            const beef = new sdk_1.Beef();
            beef.mergeRawTx(sdk_1.Transaction.fromHex(txs[0]).toBinary(), 0);
            expect(beef.isValid(false)).toBe(true);
        }
        {
            const version = 4290641921;
            expect(() => sdk_1.Beef.fromString(beefs[1]))
                .toThrow(`Serialized BEEF must start with ${exports.BEEF_MAGIC} or ${exports.BEEF_MAGIC_V2} but starts with ${version}`);
        }
    });
    ...
    test('5_mergeBeef', async () => {
        {
            const beef = sdk_1.Beef.fromString(beefs[0]);
            const beefB = sdk_1.Beef.fromString(beefs[0]);
        ...
        }
        {
            const beef = sdk_1.Beef.fromString(beefs[0]);
            const beefB = new sdk_1.Beef();
            beefB.mergeRawTx(sdk_1.Transaction.fromHex(txs[0]).toBinary(), 0);
            beef.mergeBeef(beefB);
            expect(beef.isValid()).toBe(true);
        }
        {
            const beef = sdk_1.Beef.fromString(beefs[0]);
            const beefB = new sdk_1.Beef();
            beefB.mergeTransaction(sdk_1.Transaction.fromHex(txs[0]));
            beef.mergeBeef(beefB);
            expect(beef.isValid()).toBe(true);
        }
        {
    ...
    test('6_BeefParty', async () => {
        const bp = new sdk_1.BeefParty(['b', 'c']);
...
`;
//# sourceMappingURL=Beef.test.js.map