...
class KeyDeriver {
    ...
    constructor(rootKey) {
        if (rootKey === 'anyone') {
            this.rootKey = new sdk_1.PrivateKey(1);
        }
        else {
    ...
    deriveSymmetricKey(protocolID, keyID, counterparty) {
        ...
        return new sdk_1.SymmetricKey(derivedPrivateKey.deriveSharedSecret(derivedPublicKey).x.toArray());
    }
    /**
    ...
    revealSpecificSecret(counterparty, protocolID, keyID) {
        ...
        const invoiceNumberBin = sdk_1.Utils.toArray(this.computeInvoiceNumber(protocolID, keyID), 'utf8');
        return sdk_1.Hash.sha256hmac(sharedSecret.encode(true), invoiceNumberBin);
    }
    /**
    ...
    normalizeCounterparty(counterparty) {
        if (!counterparty) {
            ...
            return new sdk_1.PrivateKey(1).toPublicKey();
        }
        else if (typeof counterparty === 'string') {
            return sdk_1.PublicKey.fromString(counterparty);
        }
        else {
            return counterparty;
        }
