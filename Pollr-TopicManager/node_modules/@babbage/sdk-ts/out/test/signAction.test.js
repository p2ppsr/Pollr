...
describe("signAction tests", () => {
    ...
    test("0_send to public key and receive back", async () => {
        var _a;
        const priv = sdk_1.PrivateKey.fromRandom();
        ...
        const r1 = await (0, src_1.createAction)({
            outputs: [{
                    satoshis,
                    script: lock.toHex(),
        ...
        const r2a = await (0, src_1.createAction)({
            inputs,
        ...
        if (!r2a.createResult)
            throw new Error();
        const ra = await (0, src_1.abortAction)({ referenceNumber: r2a.createResult.referenceNumber });
        expect(ra.referenceNumber).toBe(r2a.createResult.referenceNumber);
        const r2 = await (0, src_1.createAction)({
            inputs,
            description: `receive from private key ${priv.toHex()}`
        });
        ...
        if (!r2.createResult)
            throw new Error();
        const tx = await (0, src_1.buildTransactionForSignActionUnlocking)(inputs, r2.createResult);
        tx.inputs[0].unlockingScriptTemplate = unlock;
        await tx.sign();
        expect(tx.inputs[0].unlockingScript).toBeTruthy();
        if (!tx.inputs[0].unlockingScript)
            throw new Error();
        inputs[r1.txid].outputsToRedeem[0].unlockingScript = (_a = tx.inputs[0].unlockingScript) === null || _a === void 0 ? void 0 : _a.toHex();
        const r4 = await (0, src_1.signAction)({ inputs, createResult: r2.createResult });
        expect(r4.txid).toBeTruthy();
        expect(r4.rawTx).toBeTruthy();
    });
});
//# sourceMappingURL=signAction.test.js.map