...
export class Engine {
     ...
     */
    constructor(managers, lookupServices, storage, chainTracker, hostingURL, shipTrackers, slapTrackers, broadcaster, advertiser, syncConfiguration, logTime = false, logPrefix = '[OVERLAY_ENGINE] ', throwOnBroadcastFailure = false, overlayBroadcastFacilitator = new HTTPSOverlayBroadcastFacilitator(), logger = console) {
        ...
        for (const topic of Object.keys(this.syncConfiguration)) {
            ...
            if (Array.isArray(syncEndpoints)) {
                ...
                for (const endpoint of syncEndpoints) {
                    ...
                    try {
                        const gasp = new GASP(new OverlayGASPStorage(topic, this), new OverlayGASPRemote(endpoint, topic), 0, `[GASP Sync of ${topic} with ${endpoint}]`, true, true);
                        await gasp.sync();
        ...
        try {
            ...
            if (output.consumedBy.length === 0) {
                ...
                for (const l of Object.values(this.lookupServices)) {
                    try {
                        await l.outputDeleted?.(output.txid, output.outputIndex, output.topic);
                    }
                    catch (_) { }
                }
            }
